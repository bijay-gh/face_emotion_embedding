...
# model = Swin(num_classes=len(class_names)).to(device)
criterion = torch.nn.CrossEntropyLoss()
base_optimizer = torch.optim.Adam  # or torch.optim.SGD
optimizer = optim.SAM(model.parameters(), base_optimizer, lr=config['lr'])
...
for epoch in range(num_epochs):
    ...
    train_loss, train_acc = train_one_epoch_sam(model, train_loader, criterion, optimizer, device)
